...
class UoMCategory(models.Model):
    ...
    reference_uom_id = fields.Many2one(comodel_name='uom.uom', compute='_compute_reference_uom_id')

    def _compute_reference_uom_id(self):
        for category in self:
            category.reference_uom_id = category.uom_ids.filtered(lambda u: u.uom_type == 'reference')
