...
class TestStockMoveAutoAssign(StockMoveAutoAssignCase):
    ...
    def test_move_cancelled_with_reservation_enqueue_job(self):
        """A cancelled move with reservations enqueue a new job to assign other moves"""
    ...
    def test_move_cancelled_without_reservation_no_job(self):
        move = self._create_move(self.product, self.out_type, qty=100)
        move._action_assign()
        with mock_with_delay() as (delayable_cls, delayable):
            move._action_cancel()
